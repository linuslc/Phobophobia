		...
		public void MoveInactivePlayerToCurrentScene (int ID, Transform newTransform, _Camera associatedCamera = null)
		{
			...
			foreach (PlayerData _data in saveData.playerData)
			{
				if (_data.playerID == ID)
				{
					...
					newPlayerData.CopyData (_data);
					newPlayerData.UpdatePosition (UnityVersionHandler.GetCurrentSceneInfo (), newTransform.position, newRotation, associatedCamera);

					saveData.playerData.Remove (_data);
					saveData.playerData.Add (newPlayerData);

					PlaceAssociatedNPC (newPlayerData);
					return;
				}
			}
